...
import { users, coupons, couponRedemptions, packs } from "@/lib/db/schema";
...
export async function POST(request: Request) {
  ...
  const rateCheck = checkRateLimit(`redeem:${session.user.email}`, RATE_LIMITS.redeem);
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const updateConditions = coupon.maxUses !== null
      ? and(eq(coupons.id, coupon.id), sql`${coupons.uses} < ${coupon.maxUses}`)
